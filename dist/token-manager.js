...
TokenManager.prototype = {
  ...
  */
	isAccessTokenValid: function isAccessTokenValid(tokenInfo, bufferMS) {
  ...
  * @private
  */
	getTokens: function getTokens(formParams, options) {
		var params = {
			method: 'POST',
		...
		return this.requestManager.makeRequest(params).then(function (response) {
  ...
  * @returns {Promise<TokenInfo>} Promise resolving to the token info
  */
	getTokensAuthorizationCodeGrant: function getTokensAuthorizationCodeGrant(authorizationCode, options) {
  ...
  * @returns {Promise<TokenInfo>} Promise resolving to the token info
  */
	getTokensClientCredentialsGrant: function getTokensClientCredentialsGrant(options) {
  ...
  * @returns {Promise<TokenInfo>} Promise resolving to the token info
  */
	getTokensRefreshGrant: function getTokensRefreshGrant(refreshToken, options) {
  ...
  * @returns {Promise<TokenInfo>} Promise resolving to the token info
  */
	getTokensJWTGrant: function getTokensJWTGrant(type, id, options) {
		var _this = this;
		...
		return this.getTokens(params, options).catch(function (err) {
			...
			if (err.authExpired && err.response.body.error_description.indexOf('exp') > -1 && err.response.headers.date) {
				...
				claims.exp = serverTime + _this.config.appAuth.expirationTime;
				...
				return _this.getTokens(params, options);
			}
  ...
  */
	exchangeToken: function exchangeToken(accessToken, scopes, resource, options) {
		var params = {
  ...
  */
	revokeTokens: function revokeTokens(token, options) {
		var params = {
			method: 'POST',
