...
function constructBoxUAHeader() {
	...
	return Object.keys(analyticsIdentifiers).map(function (k) {
		return k + '=' + analyticsIdentifiers[k];
	}).join('; ');
}
...
BoxClient.prototype._makeRequest = function (params, callback) {
	...
	if (this._batch) {
		...
		promise = this._session.getAccessToken(this._tokenOptions).then(function (accessToken) {
...
BoxClient.prototype.batchExec = function (callback) {
	...
	var params = {
		body: {
			requests: this._batch.map(function (batchReq) {
	...
	this._batch = null;
	return this.post('/batch', params).then(function (res) {

		var responses = res.body.responses;

		responses.map(function (x) {
			return formatResponseForBatch(x);
