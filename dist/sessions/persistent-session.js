...
PersistentSession.prototype._refreshTokens = function (options) {
	...
	if (!this._refreshPromise) {
		...
		this._refreshPromise = this._tokenManager.getTokensRefreshGrant(this._tokenInfo.refreshToken, options).catch(function (err) {
			...
			if (err.statusCode === httpStatusCodes.BAD_REQUEST && _this._tokenStore) {
				...
				// we couldn't refresh.
				return _this._tokenStore.readAsync().catch(function (e) {
					...
					if (!storeTokenInfo || storeTokenInfo.refreshToken === _this._tokenInfo.refreshToken) {
			...
			if (_this._tokenStore) {
				return _this._tokenStore.writeAsync(tokenInfo).catch(function (e) {
					return errors.unwrapAndThrow(e);
				}).then(function () {
					return tokenInfo;
				});
			...
			return _this.handleExpiredTokensError(err);
		}).finally(function () {
...
PersistentSession.prototype.exchangeToken = function (scopes, resource, options) {
	...
	return this.getAccessToken(options).then(function (accessToken) {
		return _this2._tokenManager.exchangeToken(accessToken, scopes, resource, options);
	});
};
